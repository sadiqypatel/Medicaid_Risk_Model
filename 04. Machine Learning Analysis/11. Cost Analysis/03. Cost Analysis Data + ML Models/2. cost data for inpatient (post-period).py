...
for month_name in month_names:
  ...
  table_name = f"dua_058828.tafr17_inpatient_line_{month_name}"
...
for month_name in month_names:
  ...
  # Get the DataFrame for the current month
  table_name = f"dua_058828.tafr17_inpatient_header_{month_name}"
...
inpatient2017 = otherServices_01.union(otherServices_02).union(otherServices_03).union(otherServices_04).union(otherServices_05).union(otherServices_06).union(otherServices_07).union(otherServices_08).union(otherServices_09).union(otherServices_10).union(otherServices_11).union(otherServices_12)
...
for month_name in month_names:
  ...
  # Get the DataFrame for the current month
  table_name = f"dua_058828.tafr18_inpatient_line_{month_name}"
  df = spark.table(table_name).select("BENE_ID", "STATE_CD", "CLM_ID", "LINE_NUM", "LINE_MDCD_PD_AMT", "LINE_MDCD_FFS_EQUIV_AMT")
  df = df.dropna(subset=["BENE_ID"])
  df = df.dropna(subset=["STATE_CD"])
  exec(f"dfLine_{month_name} = df")
...
for month_name in month_names:
  ...
  # Get the DataFrame for the current month
  table_name = f"dua_058828.tafr18_inpatient_header_{month_name}"
  df = spark.table(table_name).select("BENE_ID", "STATE_CD", "CLM_ID", "CLM_TYPE_CD", "SRVC_BGN_DT", "SRVC_END_DT", "MDCD_PD_AMT")
  df = df.where((col("CLM_TYPE_CD") == '1') | (col("CLM_TYPE_CD") == '3'))
  df = df.dropna(subset=["BENE_ID"])
  df = df.dropna(subset=["STATE_CD"])  
...
inpatient2018 = otherServices_01.union(otherServices_02).union(otherServices_03).union(otherServices_04).union(otherServices_05).union(otherServices_06).union(otherServices_07).union(otherServices_08).union(otherServices_09).union(otherServices_10).union(otherServices_11).union(otherServices_12)
...
inpatient2019 = otherServices_01.union(otherServices_02).union(otherServices_03).union(otherServices_04).union(otherServices_05).union(otherServices_06).union(otherServices_07).union(otherServices_08).union(otherServices_09).union(otherServices_10).union(otherServices_11).union(otherServices_12)
print((inpatient2019.count(), len(inpatient2019.columns)))

# COMMAND ----------

inpatient_paper1_cost = inpatient2019.union(inpatient2018).union(inpatient2017)

inpatient_paper1_cost = inpatient_paper1_cost.withColumnRenamed("BENE_ID", "beneID") \
...
inpatient_merge = inpatient_paper1_cost.join(df, on=['beneID','state'], how='inner')


# Map "SRVC_BGN_DT" to a "service_month" value from 1 to 36
inpatient_merge = inpatient_merge.withColumn("service_month", 
                                       (year(col("SRVC_BGN_DT")) - 2017) * 12 + month(col("SRVC_BGN_DT")))

# Create the "post" indicator
inpatient_merge = inpatient_merge.withColumn("post", 
...
# selected = inpatient_merge.select("beneID", "state", "SRVC_BGN_DT", "enrollMonth", "enrollYear", "post_start", "post_end", "service_month", "post")
...
print(inpatient_merge.count())
filtered_df = inpatient_merge.filter(inpatient_merge["post"] == 1)
...
filtered_df.write.saveAsTable("dua_058828_spa240.paper1_stage2_inpatient_cost_15M", mode='overwrite') 