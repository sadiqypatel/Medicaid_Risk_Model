...
df_new = inpatient.withColumn("ffs_total", when(inpatient['CLM_TYPE_CD'] == '1', inpatient['MDCD_PD_AMT']).otherwise(None)) \
...
selected_columns = ["beneID", "state", "CLM_ID", "LINE_NUM", "CLM_TYPE_CD", "LINE_MDCD_FFS_EQUIV_AMT", "MDCD_PD_AMT", "ffs_total", "mc_total", "ffs_claim", "mc_claim"]
...
select beneID, CLM_ID, state, sum(mc_total) as mc_total, mean(ffs_total) as ffs_total, sum(ffs_claim) as ffs_claim, sum(mc_claim) as mc_claim
FROM missingTotal 
