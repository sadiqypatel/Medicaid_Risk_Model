...
#inpatient claims
inpatient = spark.table("dua_058828_spa240.paper1_stage2_pharm_cost")
...
inpatient_missing = inpatient.withColumn("missingFFS_header", when((col("CLM_TYPE_CD") == "1") & (col("MDCD_PD_AMT").isNull()), lit(1)).otherwise(lit(0)))
inpatient_missing = inpatient_missing.withColumn("missingFFS_line", when((col("CLM_TYPE_CD") == "1") & (col("LINE_MDCD_PD_AMT").isNull()), lit(1)).otherwise(lit(0)))
...
select distinct beneID, state, CLM_ID, max(missingFFS_header) as missingFFS_header, max(missingFFS_line) as missingFFS_line, max(missingMC) as missingMC, max(ffs_claim) as ffs_claim, max(mc_claim) as mc_claim
FROM missingTable 
...
select state, sum(missingFFS_header) as missingFFS_header, sum(missingFFS_line) as missingFFS_line, sum(missingMC) as missingMC, sum(ffs_claim) as ffs_claim, sum(mc_claim) as mc_claim, sum(totalClaims) as total
FROM missingTotal 
GROUP BY state; 
''')
...
df_new = inpatient.withColumn("ffs_header_total", when(inpatient['CLM_TYPE_CD'] == '1', inpatient['MDCD_PD_AMT']).otherwise(None)) \
           .withColumn("ffs_line_total", when(inpatient['CLM_TYPE_CD'] == '1', inpatient['LINE_MDCD_PD_AMT']).otherwise(None)) \
           .withColumn("mc_total", when(inpatient['CLM_TYPE_CD'] == '3', inpatient['LINE_MDCD_FFS_EQUIV_AMT']).otherwise(None))

df_new = df_new.fillna(0.0, subset=["ffs_header_total", "ffs_line_total", "mc_total"])
...
selected_columns = ["beneID", "state", "CLM_ID", "LINE_NUM", "CLM_TYPE_CD", "LINE_MDCD_FFS_EQUIV_AMT", "MDCD_PD_AMT", "LINE_MDCD_PD_AMT", "ffs_header_total", "ffs_line_total", "mc_total", "ffs_claim", "mc_claim"]
...
select beneID, CLM_ID, state, sum(mc_total) as mc_total, mean(ffs_header_total) as ffs_header_total, sum(ffs_line_total) as ffs_line_total, sum(ffs_claim) as ffs_claim, sum(mc_claim) as mc_claim
FROM missingTotal 
...
select state, sum(mc_total) as mc_total, sum(ffs_header_total) as ffs_header_total, sum(ffs_line_total) as ffs_line_total, sum(ffs_claim) as ffs_claim_lines, sum(mc_claim) as mc_claim_lines
FROM missingTotal 
GROUP BY state; 
''')
