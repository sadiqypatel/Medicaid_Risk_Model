...
mlflow.set_experiment("/Users/SPA240/riskScoreCode/analysis/stage1/XGboost select hyperparameter")
...
#forst grid search based on model
# param_grid = (
#     ParamGridBuilder()
#     .addGrid(xgb_classifier.learning_rate, [0.01, 0.1, 0.3, 0.5])                  #4
#     .addGrid(xgb_classifier.subsample, [0.5, 0.75, 1.0])                           #3
#     .addGrid(xgb_classifier.min_child_weight, [0, 2.5, 5])                         #3
#     .addGrid(xgb_classifier.colsample_bytree, [0.5, 0.75, 1.0])                    #3
#     .addGrid(xgb_classifier.max_depth, [4, 8, 12, 16, 20])                         #5
#     .addGrid(xgb_classifier.reg_alpha, [0.0, 0.5, 1.0])                            #3
#     .addGrid(xgb_classifier.reg_lambda, [0.0, 0.5, 1.0])                           #3
#     .addGrid(xgb_classifier.gamma, [0.0, 0.5, 1.0])                                #3
#     .addGrid(xgb_classifier.colsample_bylevel, [0.5, 0.75, 1.0])                   #3
#     .build()
# )
...
for params in param_grid:
    ...
    xgb = XgboostClassifier(featuresCol="features", labelCol="loseCoverage", missing=0.0, **{param.name: value for param, value in params.items()})
    #print(xgb_model)
