...
train_df = spark.table("dua_058828_spa240.train_xg_boost_stage1")
test_df = spark.table("dua_058828_spa240.test_xg_boost_stage1")
...
tp = train_df.filter((col('prediction') == 1.0) & (col('loseCoverage') == 1.0)).count()
tn = train_df.filter((col('prediction') == 0.0) & (col('loseCoverage') == 0.0)).count()
fp = train_df.filter((col('prediction') == 1.0) & (col('loseCoverage') == 0.0)).count()
fn = train_df.filter((col('prediction') == 0.0) & (col('loseCoverage') == 1.0)).count()
...
mcc = calculate_mcc(tn, fp, fn, tp)
auc_evaluator = BinaryClassificationEvaluator(rawPredictionCol='rawPrediction', labelCol='loseCoverage', metricName='areaUnderROC')
auc = auc_evaluator.evaluate(train_df)
