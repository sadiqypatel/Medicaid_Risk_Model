...
df = spark.table("dua_058828_spa240.stage2_random_sample_5million_vector_assembler_area_SDOH_only_new")
print(df.count())
...
#fraction = 1000000 / df.count()

# Take a random sample from the DataFrame
sampled_df1 = df.sample(withReplacement=False, fraction=fraction, seed=42)

# Show the number of rows in the sampled DataFrame
print("Number of rows in the sampled DataFrame:", sampled_df1.count())

# Continue with your analysis using the sampled DataFrame 'sampled_df'

# COMMAND ----------

sampled_df = sampled_df1.select("beneID", "state", "features", "non_emerg_binary")
...
mlflow.set_experiment("/Users/SPA240/riskScoreCode/analysis/7. stage 2 models (non-emerg)/3f. XGboost final model")
mlflow.set_tracking_uri("databricks")

# Create an instance of the XGBoostClassifier
xgb_classifier = XgboostClassifier(featuresCol="features", labelCol="non_emerg_binary")
...
#     xgb = XgboostClassifier(featuresCol="features", labelCol="non_emerg_binary", missing=0.0, **params)
#     #print(xgb_model)
...
#best_hyper = ast.literal_eval(best_hyperparams)
...
final_estimator = XgboostClassifier(featuresCol="features", labelCol='non_emerg_binary', missing=0.0, **best_hyper)
...
train_logReg_withReg = train_logReg_withReg.select('beneID', 'state', 'probability_col', 'probability', 'rawPrediction', 'prediction', 'non_emerg_binary')
...
# train_logReg_withReg.write.saveAsTable("dua_058828_spa240.train_xg_boost_stage2_nonemerg_new1", mode="overwrite")
# test_logReg_withReg.write.saveAsTable("dua_058828_spa240.test_xg_boost_stage2_nonemerg_new1", mode="overwrite")
...
top_features = list(sorted_dict.keys())[:16]
#print(top_features)

# Map transformed feature names to original field names
original_field_names = [feature.split("_Vec_")[0].replace("numeric_features_", "").replace("categorical_features_", "") for feature in top_features]
original_field_names = [feature for feature in original_field_names if feature != "state"]
...
assembler = VectorAssembler(inputCols=original_field_names, outputCol="features1")
selected_features_df = assembler.transform(sampled_df1).select("features1", "non_emerg_binary")
...
correlation_matrix = Correlation.corr(selected_features_df, "features1").head()

# Extract the correlation matrix values
correlation_values = correlation_matrix[0].toArray()

# Get the variable names
variable_names = selected_features_df.schema["features1"].metadata["ml_attr"]["attrs"]["numeric"]

# Create a set to store the processed variable pairs
processed_pairs = set()

# Create a list to store the variable correlations
variable_correlations = []

# Iterate over the correlation matrix and variable names to store the correlations
for i in range(len(variable_names)):
    for j in range(i + 1, len(variable_names)):  # Iterate from i + 1 to avoid duplicate pairs
        var1 = variable_names[i]["name"]
        var2 = variable_names[j]["name"]
        correlation = correlation_values[i, j]
        if var1 != var2 and not math.isnan(correlation):
            # Check if the pair has already been processed
            pair = (var1, var2) if var1 < var2 else (var2, var1)
            if pair not in processed_pairs:
                processed_pairs.add(pair)
                variable_correlations.append((var1, var2, correlation))

# Sort the variable correlations by absolute correlation value in descending order
sorted_correlations = sorted(variable_correlations, key=lambda x: abs(x[2]), reverse=True)

# Print the sorted variable correlations
for var1, var2, correlation in sorted_correlations:
...
# # Create a StringIndexer to convert the label column to numeric
# label_indexer = StringIndexer(inputCol="non_emerg_binary", outputCol="label")

# # Fit and transform the label column to obtain the indexed label
# indexed_df = label_indexer.fit(sampled_df1).transform(sampled_df1)

# # Create a VectorAssembler to assemble the predictor variables into a vector column
# assembler = VectorAssembler(inputCols=original_field_names, outputCol="features1")
...
# # Calculate the Chi-square test between each feature and the label
# chi_square_test = ChiSquareTest.test(assembled_df, "features1", "label").head()

# # Extract the p-values from the Chi-square test result
# p_values = chi_square_test.pValues

# # Create a dictionary to store the predictor variables and their p-values
# variable_p_values = dict(zip(original_field_names, p_values))

# # Sort the predictor variables by p-value in ascending order
# sorted_variables = sorted(variable_p_values.items(), key=lambda x: x[1])

# # Print the sorted predictor variables and their p-values
# for variable, p_value in sorted_variables:
#     print(f"P-value between {variable} and non_emerg_binary: {p_value}")