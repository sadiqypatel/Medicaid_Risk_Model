...
sampled_df = sampled_df1.select("beneID", "state", "features", "all_cause_binary")
...
final_estimator = XgboostClassifier(featuresCol="features", labelCol='all_cause_binary', missing=0.0, **best_hyper)
...
# Create a new DataFrame with only the top 16 features
assembler = VectorAssembler(inputCols=original_field_names, outputCol="features1")
selected_features_df = assembler.transform(sampled_df1).select("features1", "all_cause_binary")
...
# label_indexer = StringIndexer(inputCol="all_cause_binary", outputCol="label")
