...
train_df = spark.table("dua_058828_spa240.stage2_train_non_emerg_logReg_withReg")
...
tp = train_df.filter((col('prediction') == 1.0) & (col('non_emerg_binary') == 1.0)).count()
tn = train_df.filter((col('prediction') == 0.0) & (col('non_emerg_binary') == 0.0)).count()
fp = train_df.filter((col('prediction') == 1.0) & (col('non_emerg_binary') == 0.0)).count()
fn = train_df.filter((col('prediction') == 0.0) & (col('non_emerg_binary') == 1.0)).count()
