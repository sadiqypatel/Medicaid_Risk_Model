...
sampled_df.groupBy("all_cause_binary").count().show()
...
# Concatenate the downsampled DataFrame with all_cause_binary == 1 observations
concatenated_df = downsampled_df.union(non_emerg_1_df)

concatenated_df.groupBy("all_cause_binary").count().show()
...
rf = RandomForestClassifier(featuresCol="features", labelCol="all_cause_binary",  **best_hyper)
model = rf.fit(concatenated_df)
...
test_logReg_withReg.write.saveAsTable("dua_058828_spa240.test_rf_all_cause_down_sample_1", mode="overwrite")
...
test_logReg_withReg.write.saveAsTable("dua_058828_spa240.test_rf_all_cause_down_sample_2", mode="overwrite")
