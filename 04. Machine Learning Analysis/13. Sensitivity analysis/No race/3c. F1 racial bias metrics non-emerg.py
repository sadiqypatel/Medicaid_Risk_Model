...
test_df = spark.table("dua_058828_spa240.test_xg_nonemerg_sensitivity_no_race")
...
true_labels_col = "non_emerg_binary"

# Convert "prediction" and "non_emerg_binary" columns to integer
df = df.withColumn("prediction", col("prediction").cast("int"))
df = df.withColumn("non_emerg_binary", col("non_emerg_binary").cast("int"))
...
for race_category in race_categories:
    ...
    for _ in range(num_bootstraps):
        ...
        f1_score = calculate_f1(bootstrap_sample, 'prediction', 'non_emerg_binary', race_category)
